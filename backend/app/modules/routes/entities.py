...
class PuntoInteres(Base):
    ...
    # declarada en el modelo (app/modules/visits/entities.py) pero nunca se
    # creó en la base real ("Invalid object name 'ACTIVACIONES'", confirmado
    # en producción 2026-08-04). Declarar esa relación acá hacía que
    # SQLAlchemy intentara cargarla automáticamente al borrar CUALQUIER PDV
    # (chequeo de cascada), rompiendo el DELETE con 500 sin importar qué se
    # validara antes. app/modules/auditors/controller.py también consulta
    # Activacion directo y tiene el mismo problema pendiente -- no se toca acá.
