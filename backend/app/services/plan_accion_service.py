"""Plan de Acción (Fase 2): servicio de cálculo de visitas pendientes con score
de urgencia. Compara visitas completas (foto de activación Y de desactivación,
ninguna de las dos Rechazada -- NO exige que ya estén Aprobada por un analista,
solo que el mercaderista haya hecho el trabajo de campo; exigir aprobación
inflaba mucho el conteo porque la revisión administrativa suele ir atrasada
respecto al trabajo real) contra la cuota de FRECUENCIAS_PDVS_CLIENTE por
semana o mes según corresponda, pondera por prioridad de ruta y baja el peso si
la visita pendiente tiene fotos 'Rechazada' sin resolver, se clasifica como
'fotos_rechazadas' (peso 0.6); si nunca se intentó, 'nunca_visitado' (peso
1.0) -- misma cola, menor peso a la rechazada, tal como lo pidió el usuario.

El "universo" son las combinaciones (id_ruta, id_punto_interes, id_cliente)
activas en RUTA_PROGRAMACION. Como el piloto está en curso, muchas aún no han
tenido NI UNA visita: esas no están "atrasadas", simplemente no arrancaron, y
marcarlas pendientes inflaría el conteo (casi todas las combinaciones caerían
como 'nunca_visitado'). Por eso el UNIVERSO_QUERY excluye con EXISTS las
combinaciones que nunca arrancaron; apenas tengan su primera visita entran
solas al cálculo normal de frecuencia.
"""
