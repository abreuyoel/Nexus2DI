...
async def async_miembros_extra(db: AsyncSession, id_cliente: int, tipo_grupo: str) -> list[dict]:
...
async def async_get_miembros_grupo(db: AsyncSession, id_cliente: int, tipo_grupo: str) -> list[dict]:
    if tipo_grupo not in TIPOS_VALIDOS:
        raise ValueError(f"tipo_grupo inválido: {tipo_grupo}")
    ...
    if tipo_grupo == "encuestador":
        ...
        for r in rows:
            miembros[int(r[0])] = {"id_usuario": int(r[0]), "username": r[1], "nombre": r[2], "origen": "encuestador"}
        admins = (await db.execute(text("SELECT id_usuario, username FROM USUARIOS WHERE id_rol = 8 AND activo = 1"))).fetchall()
        for r in admins:
            if int(r[0]) not in miembros:
                miembros[int(r[0])] = {"id_usuario": int(r[0]), "username": r[1], "nombre": None, "origen": "admin"}
    else:
        bloques: list[tuple[str, dict]] = [
            ("""
                SELECT DISTINCT u.id_usuario, u.username, mm.nombre AS nombre, 'mercaderista' AS origen
                FROM MERCADERISTAS_RUTAS mr
                JOIN RUTA_PROGRAMACION rp ON rp.id_ruta = mr.id_ruta
                JOIN MERCADERISTAS mm     ON mm.id_mercaderista = mr.id_mercaderista
                JOIN USUARIOS u           ON u.username = CAST(mm.cedula AS NVARCHAR(50))
                WHERE rp.id_cliente = :cid AND rp.activa = 1 AND u.activo = 1
            """, {"cid": id_cliente}),
            ("""
                SELECT DISTINCT u.id_usuario, u.username, an.nombre_analista AS nombre, 'analista' AS origen
                FROM analistas_rutas ar
                JOIN RUTA_PROGRAMACION rp ON rp.id_ruta = ar.id_ruta
                JOIN ANALISTAS an         ON an.id_analista = ar.id_analista
                JOIN USUARIOS u           ON u.id_perfil = ar.id_analista
                WHERE u.id_rol = 2 AND rp.id_cliente = :cid AND rp.activa = 1 AND u.activo = 1
            """, {"cid": id_cliente}),
            ("""
                SELECT DISTINCT u.id_usuario, u.username, COALESCE(an.nombre_analista, cl.cliente, me.nombre) AS nombre, 'coordinador' AS origen
                FROM USUARIOS u
                ...
                WHERE u.id_rol IN (3, 4, 8, 11) AND u.activo = 1
            """, {}),
        ]

        if tipo_grupo == "operativo_cliente":
            bloques.append(("""
                SELECT DISTINCT u.id_usuario, u.username, cl.cliente AS nombre, 'cliente' AS origen
                FROM USUARIOS u
                LEFT JOIN CLIENTES cl ON cl.id_cliente = u.id_perfil AND u.id_rol = 1
                WHERE u.id_rol = 1 AND u.id_perfil = :cid AND u.activo = 1
            """, {"cid": id_cliente}))
        ...
        for sql, params in bloques:
            ...
            for row in rows:
                ...
                if uid is None or uid in miembros:
                    continue
                miembros[int(uid)] = {"id_usuario": int(uid), "username": row[1], "nombre": row[2], "origen": row[3]}

    extra_m = await async_miembros_extra(db, id_cliente, tipo_grupo)
    for m in extra_m:
        miembros.setdefault(m["id_usuario"], m)
...
async def async_get_miembros_ids(db: AsyncSession, id_cliente: int, tipo_grupo: str) -> set[int]:
    miembros = await async_get_miembros_grupo(db, id_cliente, tipo_grupo)
    return {m["id_usuario"] for m in miembros}


async def async_usuario_es_miembro(db: AsyncSession, id_usuario: Optional[int], id_cliente: int, tipo_grupo: str) -> bool:
    if id_usuario is None:
        return False
    ids = await async_get_miembros_ids(db, id_cliente, tipo_grupo)
    return int(id_usuario) in ids


async def async_asegurar_grupos_cliente(db: AsyncSession, id_cliente: int, cliente_nombre: Optional[str] = None) -> int:
    if cliente_nombre is None:
...
async def async_asegurar_grupo_encuestadores(db: AsyncSession) -> Optional[int]:
    row = (await db.execute(text("""
        SELECT id_grupo FROM CHAT_GRUPOS WHERE id_cliente = :cid AND tipo_grupo = 'encuestador'
    """), {"cid": ID_CLIENTE_ENCUESTADORES})).fetchone()
    if row:
...
async def async_grupos_extra_de_usuario(db: AsyncSession, id_usuario: int) -> list[dict]:
    rows = (await db.execute(text("""
        SELECT g.id_grupo, g.id_cliente, g.tipo_grupo, g.nombre
        FROM CHAT_GRUPO_MIEMBROS_EXTRA x
        JOIN CHAT_GRUPOS g ON g.id_grupo = x.id_grupo AND g.activa = 1
...
async def async_get_grupos_de_usuario(db: AsyncSession, id_usuario: Optional[int]) -> list[dict]:
    if id_usuario is None:
        return []

    from app.services.cache_service import cache_get, cache_set
    cache_key = f"nexus2di:cache:user_grupos:{id_usuario}"
    ...
    id_perfil, id_rol = u[1], u[2]
    extra = await async_grupos_extra_de_usuario(db, id_usuario)

    if id_rol in ROLES_ENCUESTADOR:
        id_grupo_enc = await async_asegurar_grupo_encuestadores(db)
        grupos_enc = [{
            "id_grupo": id_grupo_enc, "id_cliente": ID_CLIENTE_ENCUESTADORES,
            "tipo_grupo": "encuestador", "nombre": NOMBRE_GRUPO_ENCUESTADORES,
        }] if id_grupo_enc else []
        vistos = {g["id_grupo"] for g in grupos_enc}
        res = grupos_enc + [g for g in extra if g["id_grupo"] not in vistos]
        await cache_set(cache_key, res, ttl_seconds=120)
        return res
    ...
    async def _existentes() -> dict[tuple[int, str], tuple]:
        rows = (await db.execute(text("""
            SELECT id_grupo, id_cliente, tipo_grupo, nombre FROM CHAT_GRUPOS WHERE activa = 1
        """))).fetchall()
        return {(int(cli), tipo): (int(id_grupo), nombre) for id_grupo, cli, tipo, nombre in rows}

    existentes = await _existentes()
    ...
    if faltantes:
        for cli in faltantes:
            try:
                await async_asegurar_grupos_cliente(db, cli)
            except Exception:
                pass
        existentes = await _existentes()
    ...
    vistos = {g["id_grupo"] for g in grupos}
    grupos += [g for g in extra if g["id_grupo"] not in vistos]
    await cache_set(cache_key, grupos, ttl_seconds=120)
    return grupos
